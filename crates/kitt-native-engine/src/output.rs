...
fn effective_command(argv: &[String]) -> String {
        ...
        && argv.get(1).map(String::as_str) == Some("-m")
    {
        return argv
            .get(2)
            .map(|v| normalized_program(v))
            .unwrap_or_default();
    }
    if matches!(first.as_str(), "uv" | "poetry" | "pipenv" | "rye")
        && argv.get(1).map(|v| v.eq_ignore_ascii_case("run")) == Some(true)
    {
...
fn command_has(argv: &[String], needle: &str) -> bool {
    argv.iter().any(|a| a.eq_ignore_ascii_case(needle))
}
...
fn compress_git_diff(raw: &str) -> (String, usize) {
    ...
    for line in raw.lines() {
        ...
        if is_diff_metadata(line) || changed {
            selected.push(line);
        }
    }
...
fn compress_build(raw: &str, success: bool) -> (String, usize) {
    ...
    if success {
        for (index, line) in lines.iter().enumerate().rev().take(80) {
            if success_summary(&line.to_ascii_lowercase()) {
        ...
        if indexes.is_empty() {
            indexes.extend(lines.len().saturating_sub(20)..lines.len());
        }
    } else {
        ...
        if indexes.is_empty() {
            indexes.extend(lines.len().saturating_sub(48)..lines.len());
        }
    }
    let selected: Vec<&str> = indexes
        .iter()
...
pub fn compress_with_budget(
    ...
    let fam = family(argv);
    if raw.is_empty()
        || estimate_tokens(&raw) <= token_budget.clamp(MIN_TOKEN_BUDGET, MAX_TOKEN_BUDGET)
    {
...
mod tests {
    ...
    fn successful_pytest_keeps_summary_not_pass_spam() {
        ...
        assert!(!r.output.contains("tests/test_499.py"));
    }
    ...
    fn wrapper_command_is_classified() {
        let raw = (0..300)
            .map(|i| format!("tests/test_{i}.py::test_case PASSED"))
            .chain(std::iter::once("300 passed in 1.20s".to_string()))
            .collect::<Vec<_>>()
            .join("\n");
        let r = compress_with_budget(
            &["uv".into(), "run".into(), "pytest".into(), "-v".into()],
            &raw,
            "",
            0,
            128,
        );
        assert_eq!(r.family, "build_test");
        assert!(r.output.contains("300 passed"));
    }
    ...
    fn git_diff_preserves_duplicate_changed_lines() {
        ...
        assert_eq!(r.output.matches("-same").count(), 2);
        assert_eq!(r.output.matches("+same").count(), 2);
    }
    ...
    fn search_counts_matches_in_non_emitted_files() {
        let raw = (0..60)
            .map(|i| format!("file-{i}.py:1:match"))
            .collect::<Vec<_>>()
            .join("\n");
